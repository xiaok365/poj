...
Node *new_node(int l, int r, int cnt) {
    Node *t = &node[tot++];
    t->l = l, t->r = r, t->cnt = cnt;
    return t;
}
...
inline void insert(int i, int l, int r, int d) {
    version[i] = new_node(l, r, version[i - 1]->cnt + 1);
    ...
    while (l < r) {
        ...
        if (d <= mid) {
            ...
            cur->lson = new_node(l, mid, pre_cnt);
            cur = cur->lson, pre = pre->lson;
            r = mid;
        } else {
            cur->lson = pre->lson;
            pre_cnt = pre->rson ? pre->rson->cnt + 1 : 1;
            cur->rson = new_node(mid + 1, r, pre_cnt);
